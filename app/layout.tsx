              ...
              <div className={"bg-[#121212] w-full rounded-lg px-6 py-5 space-y-2"}>
                <div className={"flex flex-row items-center space-x-0.5 pb-1"}>
                  <div className="w-6 h-6 flex items-center justify-center">
                    <svg width="24" height="24" viewBox="0 0 760 696" fill="#121212" xmlns="http://www.w3.org/2000/svg">
                      <path
                ...
                {
                  MENU.map((item) => (
                    <Link href={`/${item.path[0]}`} prefetch
                      className={`flex flex-row items-center h-10 space-x-5 ${item.path.includes(pathname.split("/")[1]) ? "text-white" : "text-[#B3B3B3]"}`} key={item.label}>
                      {
                        item.path.includes(pathname.split("/")[1]) ? item.selectedIcon : item.icon
                      }
                      <div className={"font-medium select-none"}>
                        {item.label}
                      </div>
